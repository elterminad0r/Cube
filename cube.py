...
# read the cube template, parsing into a formattable string
with open("cube_template", "r") as template:
    CUBE_TEMP = ""
    for line in template:
        CUBE_TEMP += re.sub(r"([URFLDB])([0-8])",
                             lambda match: "{{{}}}".format(
                                get_pos(FACE_TO_NUM[match.group(1)],
                                        int(match.group(2)))
                                 ),
                             line)
